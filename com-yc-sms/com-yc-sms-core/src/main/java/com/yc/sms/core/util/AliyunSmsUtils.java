...
import com.yc.sms.core.config.SmsRefreshProperties;
...
public class AliyunSmsUtils {
    ...
    public String sendMsg(List<Object> telephone, String message, String templateCode) throws SmsException,ClientException {
        if (telephone==null || telephone.size()==0){
            throw new SmsException(SmsExceptionCodeEnum.PHONE_NULL);
        }
        String bizId = null;
        try {
            //设置超时时间-可自行调整
            ...
            if(sendSmsResponse.getCode() == null || !sendSmsResponse.getCode().equals("OK")) {
                ...
                throw new SmsException(SmsExceptionCodeEnum.SEND_FAIL);
            }
            //请求成功
            bizId = sendSmsResponse.getBizId();
        }catch (ClientException e){
            log.error(e.getMessage(),e);
            throw new SmsException(SmsExceptionCodeEnum.SEND_REQUEST_FAIL);
        }
        return bizId;
    }
